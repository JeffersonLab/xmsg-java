...
public final class LocalThroughput {
    ...
    public static void main(String[] argv) {
        ...
        try {
            final xMsg subscriber = new xMsg("localhost", 1);
            final xMsgAddress address = new xMsgAddress(bindTo);
            final xMsgConnection connection = subscriber.getNewConnection(address);
            final String topic = "thr_topic";

            xMsgUtil.sleep(100);

            SubscriptionHandler sub = subscriber.subscribe(connection, topic, new xMsgCallBack() {

                @Override
                public xMsgMessage callback(xMsgMessage msg) {
                    Data.Builder data = (Data.Builder) msg.getData();
